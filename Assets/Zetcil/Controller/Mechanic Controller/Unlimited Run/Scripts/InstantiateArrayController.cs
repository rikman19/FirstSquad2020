...
namespace Zetcil
{
    ...
    public class InstantiateArrayController : MonoBehaviour
    {
        ...
        public void InvokeInstantiateController()
        {
            if (usingParent)
            {
                GameObject temp = Instantiate(TargetPrefab[CurrentIndex], TargetPosition.position, TargetPosition.rotation, TargetParent);
            ...
            {
                GameObject temp = Instantiate(TargetPrefab[CurrentIndex], TargetPosition.position, TargetPosition.rotation);
                if (temp == null) Debug.Log("Instantiate Failed.");
            }
            CurrentIndex++;
            if (CurrentIndex > TargetPrefab.Count - 1)
            {
                CurrentIndex = 0;
            }
        }
