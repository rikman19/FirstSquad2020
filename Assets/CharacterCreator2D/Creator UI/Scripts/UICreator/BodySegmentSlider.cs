...
namespace CharacterCreator2D.UI
{
    public class BodySegmentSlider : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            if (_uicreator == null || slider == null)
                return;

            switch (sliderType)
            {
                case BodySliderType.X:
                    slider.value = _uicreator.character.GetBodySlider(segmentType).x;
                    break;
                case BodySliderType.Y:
                    slider.value = _uicreator.character.GetBodySlider(segmentType).y;
                    break;
